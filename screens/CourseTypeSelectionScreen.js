...
import { getVerifiedSkills } from '../utils/skillVerification';
...
const CourseTypeSelectionScreen = () => {
  ...
  const handleCategorySelect = async (category) => {
    ...
    // Check if the user has verified skills in this category
    const verifiedSkills = await getVerifiedSkills();
    const hasVerifiedSkillsInCategory = Object.keys(verifiedSkills).length > 0;
    
    if (hasVerifiedSkillsInCategory) {
      // Navigate to course creation with the selected category
      setTimeout(() => {
        navigation.navigate('CourseCreation', { category });
      }, 300);
    } else {
      // User needs to verify skills first
      setTimeout(() => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          <Text style={styles.title}>Choose Course Category</Text>
          <Text style={styles.subtitle}>
            Select the category for your new course. You must have verified skills in this category.
          </Text>
...
export default CourseTypeSelectionScreen; 